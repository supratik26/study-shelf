...
const navigation = [
  ...
  { href: "/my-notes", label: "My Notes", icon: NotebookPen },
];
    ...
    <header className="site-header archive-header sticky top-0 z-40">
      <div className="container flex h-19 items-center justify-between gap-5">
        ...
        <nav className="hidden items-center gap-1.5 md:flex" aria-label="Primary navigation">
          {visibleNavigation.map(item => { const Icon = item.icon; const active = location === item.href; return <Link key={item.href} href={item.href} className={cn("archive-nav-link motion-surface motion-press", active && "archive-nav-link--active")} aria-current={active ? "page" : undefined}><Icon className="h-3.5 w-3.5" /><span>{item.label}</span></Link>; })}
        </nav>
        ...
        <div className="flex items-center gap-2 md:hidden"><button className="theme-toggle motion-press" type="button" onClick={toggleTheme} aria-label={theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}>{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button><button className={cn("site-menu-button motion-press grid h-10 w-10 place-items-center rounded-full border border-[#151c4a]/20 text-[#151c4a]", isOpen && "site-menu-button--open")} onClick={() => setIsOpen(open => !open)} aria-label={isOpen ? "Close navigation" : "Open navigation"} aria-expanded={isOpen}>{isOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button></div>
      </div>
      {isOpen && <div className="site-mobile-menu border-t border-[#151c4a]/12 px-4 pb-5 pt-3 md:hidden"><nav className="mx-auto flex max-w-7xl flex-col gap-1" aria-label="Mobile navigation">{visibleNavigation.map(item => { const Icon = item.icon; const active = location === item.href; return <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} className={cn("site-mobile-menu-item motion-surface motion-press rounded-xl px-4 py-3 text-base font-semibold", active ? "site-mobile-menu-item--active" : "site-mobile-menu-item--inactive")} aria-current={active ? "page" : undefined}><Icon className="h-4 w-4" /><span>{item.label}</span></Link>; })}<div className="mt-2 border-t border-[#151c4a]/12 pt-3"><p className="mb-2 text-xs font-bold uppercase tracking-[0.14em] text-[#151c4a]/58">Colour theme</p><ThemePicker expanded /><div className="mt-3">{isAuthenticated ? <button className="editorial-text-button w-full justify-center" onClick={() => void handleLogout()}><LogOut className="h-3.5 w-3.5" />Sign out {user?.name ? `(${user.name})` : ""}</button> : <button className="editorial-button editorial-button--amber w-full justify-center" onClick={() => void handleSignIn()} disabled={signInPending}>{signInPending ? <><Loader2 className="h-4 w-4 animate-spin" />Opening Google…</> : <><Upload className="h-4 w-4" />Sign in to study</>}</button>}</div></div></nav></div>}
    </header>
  );
}
