    ...
    <header className="site-header archive-header sticky top-0 z-40">
      <div className="container flex h-19 items-center justify-between gap-5">
        ...
        <nav className="hidden items-center gap-1.5 md:flex" aria-label="Primary navigation">
          {visibleNavigation.map(item => { const Icon = item.icon; const active = location === item.href; return <Link key={item.href} href={item.href} className={cn("archive-nav-link", active && "archive-nav-link--active")} aria-current={active ? "page" : undefined}><Icon className="h-3.5 w-3.5" /><span>{item.label}</span></Link>; })}
        </nav>
