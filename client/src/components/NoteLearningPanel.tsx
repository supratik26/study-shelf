...
export default function NoteLearningPanel({ note, collections }: { note: ExternalNote; collections: Collection[] }) {
  ...
  return <section className="note-learning-panel"><div className="note-learning-panel__header"><span className="workspace-kicker"><Lightbulb className="h-3.5 w-3.5" />Study actions</span><h2>Use the material with intention.</h2></div><div className="note-action-grid"><button type="button" onClick={() => { setPreviewOpen(true); void preview.refetch(); }}><Eye className="h-4 w-4" />Preview in browser</button><button type="button" onClick={() => addQueue.mutate()} disabled={addQueue.isPending}><BookmarkPlus className="h-4 w-4" />Add to queue</button><button type="button" onClick={() => void saveOffline()}><WifiOff className="h-4 w-4" />Save locally</button></div>{collections.length ? <div className="note-collection-row"><FolderHeart className="h-4 w-4" /><select value={collectionId} onChange={event => setCollectionId(event.target.value)} aria-label="Choose collection"><option value="">Save to a collection…</option>{collections.map(collection => <option value={collection.id} key={collection.id}>{collection.name}</option>)}</select><button type="button" onClick={() => collectionId && addCollection.mutate()} disabled={!collectionId || addCollection.isPending}>Save</button></div> : <Link href="/study-space" className="note-create-collection"><FolderHeart className="h-4 w-4" />Create a collection first</Link>}
    {previewOpen && <div className="note-preview-shell"><div><strong>Preview: {note.originalFileName}</strong><button type="button" onClick={() => setPreviewOpen(false)}>Close</button></div>{preview.isLoading ? <p><Loader2 className="h-4 w-4 animate-spin" />Preparing a private preview…</p> : preview.error ? <p>Preview is unavailable for this material. Download it to read the original file.</p> : preview.data?.fileType === "pdf" ? <iframe src={preview.data.previewUrl} title={`Preview of ${note.title}`} /> : <a href={preview.data?.previewUrl} target="_blank" rel="noreferrer">Open the original file in a new tab</a>}</div>}
